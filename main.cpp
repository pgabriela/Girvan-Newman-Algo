...
///////////////////////////////////////////////////////////////////////////
void printDecomposition(Matrix* copymat, Node* nodes[], int &clusterNum){
...
void modularity(Matrix* originalMat, Matrix* copymat, Node* nodes[], int &clusterNum, int &search, double &maxMod){
	...
	for(int i=0; i<B->row; i++){									//
		...
		for(int x=0; x<copymat->col; x++){							// Calculating ki(degree of node i)
			ki += originalMat->data[nodes[i]->pos][x];				//
		}															//
		for(int j=0; j<B->col; j++){								//
			double kj=0;											//
			for(int y=0; y<copymat->col; y++){						// Calculating kj(degree of node j)
				kj += originalMat->data[nodes[j]->pos][y];			//
			}														//
	...
	cout<<clusterNum<<" clusters: modularity "<<Q<<endl;
...
int main() {
	...
	for(; clusterNum<copymat->row ; ){
		...
		printDecomposition(copymat, nodes, clusterNum);								//print the decomposition
	}
	...
	for(; clusterNum<copymat->row ; ){
		totalBetweennessMat = betweenness(copymat, nodes, totalBetweennessMat);		//compute the total betweenness of all edges
		decompose(copymat, nodes, totalBetweennessMat, clusterNum);					//decomposition
		modularity(mat, copymat, nodes, clusterNum, search, maxMod);				//print the modularity
	}
